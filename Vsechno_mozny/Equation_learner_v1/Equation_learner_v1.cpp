        ...
        for (int wb = 0; wb < koef.size(); wb++)
        {
            for (int l = 0; l < koef.at(wb).size(); l++)
            {
                for (int n = 0; n < koef.at(wb).at(l).size(); n++) // projed celou naslednou verstvu
                {
                    ...
                    v.at(wb).at(l).at(n) = (beta2 * v.at(wb).at(l).at(n)) + ((beta2_1)*pow2(d_koef));
...
int main()
{
    ...
    std::vector<double> x = generate(-2.0, 2.0, 300);
   ...
   // plot(x, y0, 20);
    ...
    koef = batch_gradient_descent(y0, x, net, initialize_koeficients(net, 1.0, 0.0), 0.5, 0.01, 1000);
